...
import { navActive, PROPERTIES_NAV, SECTOR_NAV } from "@/lib/site-nav";
    ...
    <header className="sticky top-0 left-0 right-0 z-50 w-full border-b border-gray-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-2.5 sm:px-6 sm:py-3.5 lg:px-10">
        <Link className="flex min-w-0 items-center gap-2" href="/">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#1e3a5f] sm:h-10 sm:w-10">
            <HomeIcon className="h-5 w-5 text-white sm:h-6 sm:w-6" />
          </div>
          <span className="truncate text-base font-bold text-black sm:text-xl">{BRAND_NAME}</span>
        </Link>

        <nav className="hidden items-center gap-1 xl:flex">
          {PROPERTIES_NAV.filter((item) => item.href !== "/hotels").map((item) => (
        ...
        <div className="hidden items-center lg:flex" role="group" aria-label="Sign in or sign up">
          {user ? (
            <div className="flex items-center gap-3">
              <Link href={dashHref} className="text-sm font-medium text-slate-700 hover:text-[#1e3a5f]">
                {user.fullName.split(" ")[0]}
              </Link>
              <button
                type="button"
                onClick={logout}
                className="cursor-pointer rounded-md border border-black px-4 py-2 text-sm font-medium text-black transition-colors hover:bg-slate-50"
              >
                Log out
              </button>
            </div>
      ...
      {/* Tablet chip strip when desktop nav is hidden */}
            ...
            {user ? (
              <>
                ...
                <button
                  type="button"
                  onClick={logout}
                  className="rounded-lg px-3 py-2.5 text-left text-sm font-medium text-black hover:bg-[#1e3a5f]/5"
                >
                  Log out
                </button>
              </>
